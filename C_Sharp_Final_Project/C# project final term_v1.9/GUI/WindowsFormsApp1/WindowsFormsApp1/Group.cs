    ...
    public partial class Group : Form
    {
        ...
        private void LoadMatchedUsers()
        {
            ...
            string query = @"
                ...
                INNER JOIN [User] u
                ON (
                    (m.User1Id = @MyId AND u.UserId = m.User2Id)
                    OR
                    (m.User2Id = @MyId AND u.UserId = m.User1Id)
                )";

            using (SqlConnection con = new SqlConnection(connectionString))
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                cmd.Parameters.Add("@MyId", SqlDbType.Int).Value = StudentLogin.LoggedInUserId;
        ...
        private void LoadMyGroups()
        {
            ...
            string query = @"
                SELECT DISTINCT pg.GroupId, pg.ProjectTitle
                FROM ProjectGroup pg
                INNER JOIN GroupMember gm ON pg.GroupId = gm.GroupId
                WHERE gm.UserId = @UserId";

            using (SqlConnection con = new SqlConnection(connectionString))
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                cmd.Parameters.Add("@UserId", SqlDbType.Int).Value = StudentLogin.LoggedInUserId;
        ...
        private void createButton_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    SqlCommand creator = new SqlCommand(@"
                        INSERT INTO GroupMember (GroupId, UserId, Role)
                        VALUES (@g,@u,'Creator')", con, tr);
        ...
        private void rateButton_Click(object sender, EventArgs e)
        {
            ...
            GroupItem g = (GroupItem)listBox1.SelectedItem;
            int groupId = Convert.ToInt32(g.Value);

            this.Hide();
            new RateTeamMate(groupId).Show();
        }
